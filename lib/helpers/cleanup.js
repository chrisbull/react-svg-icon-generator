...
function cleanupSvg(svg, keepFillColor, keepStrokeColor) {
  ...
  if (!keepFillColor) {
    cleanedSvg.replace(/fill="#?\w+"/g, '').replace(/\<Path/g, '<Path fill={color}').replace(/\s{2,}/g, ' ').replace(/ \>/g, '>');
  }

  if (!keepStrokeColor) {
    cleanedSvg.replace(/stroke="#?\w+"/g, '').replace(/\<Path/g, '<Path stroke={color}').replace(/\s{2,}/g, ' ').replace(/ \>/g, '>');
  }
